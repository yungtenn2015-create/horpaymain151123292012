...
-- ฟังก์ชันเช็คว่า user ดูแลห้องนี้ไหม (กัน Infinite Recursion)
...
-- ฟังก์ชันเช็คว่า user เป็นผู้เช่าห้องนี้ไหม (กัน Infinite Recursion)
CREATE OR REPLACE FUNCTION is_tenant_of_room(p_room_id uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
STABLE
...
-- ------------------------------------------------------------
-- TABLE: dorm_services
-- เข้าถึงได้ถ้าเป็นเจ้าของ dorm นั้น (ใช้วิธีเดียวกับ dorm_settings)
-- ------------------------------------------------------------
ALTER TABLE dorm_services ENABLE ROW LEVEL SECURITY;

CREATE POLICY "dorm_services: เจ้าของเห็นได้"
  ON dorm_services FOR SELECT
  USING (is_dorm_owner(dorm_id));

CREATE POLICY "dorm_services: เจ้าของสร้างได้"
  ON dorm_services FOR INSERT
  WITH CHECK (is_dorm_owner(dorm_id));

CREATE POLICY "dorm_services: เจ้าของแก้ได้"
  ON dorm_services FOR UPDATE
  USING (is_dorm_owner(dorm_id))
  WITH CHECK (is_dorm_owner(dorm_id));

CREATE POLICY "dorm_services: เจ้าของลบได้"
  ON dorm_services FOR DELETE
  USING (is_dorm_owner(dorm_id));
...
CREATE POLICY "tenants: เจ้าของจัดการได้"
  ON tenants FOR ALL
  USING (is_room_owner(room_id) AND deleted_at IS NULL)
  WITH CHECK (is_room_owner(room_id) AND deleted_at IS NULL);
