...
### API ที่ใช้ service role — ต้องไม่เปิดให้คนนอกเรียกฟรี
- **`/api/line/send-bill`**, **`/api/line/confirm-payment`**, **`/api/line/notify-owner-verify`** — บังคับ `Authorization: Bearer <access_token>` และตรวจว่า user เป็น **owner ของหอที่บิลสังกัด** (`lib/verify-bill-owner-request.ts`)
- **`/api/debug/logs`** — ต้องตั้ง `DEBUG_LOGS_SECRET` ใน env และส่ง `Authorization: Bearer <DEBUG_LOGS_SECRET>` ถ้าไม่ตั้ง secret = route คืน 404 (ปิดทางสำรวจ)
- **`/api/test-supabase`** — ปิดใน production (`NODE_ENV=production` → 404)

### `trg_protect_users_privileged_columns` — กันยกระดับแผนเอง (v19)
```
BEFORE UPDATE ON public.users
JWT role = authenticated → ห้ามเปลี่ยน plan_type, subscription_plan, subscription_ends_at,
  trial_expires_at, role, email (แก้ name / phone ได้ตามเดิม)
JWT role = service_role → อนุญาตทุกคอลัมน์ (API ฝั่ง server / webhook)
ไม่มี JWT แบบ client (เช่น SQL Editor) → ไม่บล็อก — แก้มือใน Dashboard ได้
```
รัน `db/migration_v19_protect_users_privileged_columns.sql` บน DB เดิม — โปรเจกต์ใหม่ได้ฟังก์ชันนี้จาก `migration.sql` แล้ว
...
ระบบใช้ **trial 30 วัน** แทนการจำกัดจำนวนหอ/ห้อง — ในช่วง trial ใช้งานได้เต็มที่ไม่จำกัด
...
```
trial_expires_at      = now() + 30 days  ← เซตอัตโนมัติตอน signup (ฝั่ง free)
plan_type             = 'free' / 'pro'
subscription_plan     = 'monthly' / 'yearly'  ← เซตตอนอัปเกรด (metadata)
subscription_ends_at  = TIMESTAMPTZ NULL   ← Pro แบบมีวันหมดรอบ; NULL = Pro ไม่กำหนดวันหมด
```

### Helper Function `is_trial_active()`
```sql
-- คืน true ถ้ามีสิทธิ์เขียนข้อมูล: free + trial ยังไม่หมด, หรือ pro + (ไม่มีวันหมด หรือยังไม่ถึงวันหมด)
SELECT is_trial_active(user_id);
```
trigger `check_plan_limit` เรียก function นี้ทุกครั้งที่ INSERT/UPDATE dorms หรือ rooms

### อัปเกรด Pro แบบมีกำหนด (ทดสอบ / หลังรัน `migration_v18_subscription_ends_at.sql`)
```sql
UPDATE public.users
SET plan_type = 'pro',
...
```
'trial_expired: Trial 30 วันหมดแล้ว — อัปเกรดเป็น Pro เพื่อใช้งานต่อ'
```
